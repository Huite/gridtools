...
def downsample_2d(src, w, h, method=DS_MEAN, fill_value=None, mode_rank=1, out=None, src_transform=None, out_transform=None):
    ...
    if isinstance(method, CPUDispatcher):
        return _mask_or_not(_downsample_2d_func(src, mask, use_mask, method, fill_value, out, use_transform, src_transform, out_transform), src, fill_value)
    else:
...
@jit(nopython=True)
def _downsample_2d_func(src, mask, use_mask, func, fill_value, out, use_transform, src_transform, out_transform):
    # np.zeros is as default necessary for numba, it tries to unpack src_transform and out_transform
    src_w = src.shape[-1]
    src_h = src.shape[-2]
    out_w = out.shape[-1]
